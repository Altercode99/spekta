...
$script = <<< "JS"
	...
	function showSuperiorSallary() {
        var editSallaryForm;

        var sallaryLayout = mainTab.cells("superior_sallary").attachLayout({
            pattern: "2U",
            cells: [{
                    id: "a",
                    header: false
        ...
        var sallaryToolbar = mainTab.cells("superior_sallary").attachToolbar({
        ...
        var sallaryMenu =  mainTab.cells("superior_sallary").attachMenu({
            icon_path: "./public/codebase/icons/",
            items: [
        ...
        sallaryGrid.attachHeader("#rspan,#text_filter,#text_filter,#text_filter,#text_filter,#select_filter,#select_filter,#select_filter,#select_filter,#select_filter,#select_filter,#select_filter,#text_filter")
        sallaryGrid.setColSorting("int,str,str,str,str,str,str,str,str,str,str,str,str");
        sallaryGrid.setColTypes("rotxt,rotxt,edn,ron,edn,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt,rotxt");
        ...
        sallaryGrid.init();
        isGridNumeric(sallaryGrid, [3,4]);

        function setGridDP() {
            sallaryGridDP = new dataProcessor(Sallary('updateSuperSallaryBatch'));
        ...
        function rSallaryGrid() {
            ...
            sallaryGrid.clearAndLoad(Sallary("superSallaryGrid", {search: sallaryToolbar.getValue("search"), lt_rank_id: 7}), sallaryGridCount);
        }
        ...
        function editSallaryHandler() {
            ...
            editSallaryForm = sallaryLayout.cells("b").attachForm([
                {type: "fieldset", offsetTop: 30, offsetLeft: 30, label: "Update Data Gaji", list: [
                    ...
                    {type: "input", name: "premi_overtime", label: "Premi Lembur", labelWidth: 130, inputWidth:250, required: true, validate:"ValidNumeric"},
            ...
            fetchFormData(Sallary("superSallaryForm", {equal_id: sallaryGrid.getSelectedRowId()}), editSallaryForm);
            editSallaryForm.attachEvent("onButtonClick", function(name) {
                switch (name) {
                    case "update":
                        ...
                        let editSallaryFormDP = new dataProcessor(Sallary("updateSuperSallary"));
